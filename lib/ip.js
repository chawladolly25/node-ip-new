...
ip.isPublic = function (addr) {
  // Update logic to handle edge cases and non-standard IP representations
  if (ip.isPrivate(addr)) {
    return false;
  }

  // Handle edge cases for IPv4
  if (ip.isV4Format(addr)) {
    const normalizedAddr = ip.normalizeToLong(addr);
    if (normalizedAddr === -1) {
      return false;
    }
  ...
  // Handle edge cases for IPv6
  if (ip.isV6Format(addr)) {
    addr = addr.toLowerCase();
    if (/^::f{4}:127\./.test(addr) || /^::1$/.test(addr) || /^::$/.test(addr)) {
      return false;
    }
  }
